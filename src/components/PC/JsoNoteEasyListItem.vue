<template>
  <div class="content" ref="editor" @click="hanldeItemClick">
    ...
    <template v-if="contentList && contentList.length > 0">
          <div class="content-item" :ref="index + '-v'" v-for="(item, index) in contentList" :key="item.updateTime" v-if="item.text" :class="currentInput === index ? 'active' : ''" :contenteditable="status === 'show' ? 'false' : 'true'"
    ...
    methods: {
      ...
      handleItemBlur(item, index) {
        ...
        if (item.type === 'code') {
          ...
          // 常规模块失去焦点 => 正常赋值
          item.text = this.convertToText(this.$refs[index][0].innerHTML)
        }
        // 强制刷新页面
        this.updateItem(item)
        this.$forceUpdate()
        ...
        if (index !== -1) {
          ...
          this.$nextTick(() => {
            if (item.type === 'code') {} else {
              this.$refs[index][0].innerHTML = this.convertToHtml(item.text + ' ')
            }
          })
        })
        }
